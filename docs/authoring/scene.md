# 編寫 `packages/<名稱>/scene.json`

給 AI 與人類編輯者的指南。本檔每個欄位都對應引擎(`play.html`)實際消費的資料,或 `schema/scene.schema.json` 的契約。**不要發明欄位**——本指南沒列到的欄位,引擎會默默忽略,validator 也可能直接退件。
...
這份檔案以前是寫死在 `play.html` 的 `PHASES` 常數;外部化後它成為戰場 package 的一層,可被 AI 編輯。`battlefield.json` 用 `data.scene` 指向本檔。
...
| `dur` | number | 該幕**上限**長度(秒)。自動播映時引擎會在「旁白播完 + 單位停 + 洪水漲完 + 事件都觸發」靜止約 1.8s 後**提早換幕**(絕不超過 `dur`),所以 `dur` 設寬鬆就好、**不必精算幕長**。 |
| `hold` | bool | (選填)設 `true` 則該幕**不提早換**、乖乖等到 `dur` —— 留給需要刻意停頓的幕。 |
...
陣列,元素是該幕要做側舷齊射(broadside,船身轉成側對開火姿態)的**單位 id**。進幕時引擎先把所有單位的 `combat` 清掉,再對本欄列出的單位設 `combat=true`;這些單位在沒有 `path` 行進時會把船首轉向最近的 0 或 π(側身),營造對轟。取代了以前寫死在 `play.html` 的 `COMBAT_FLEETS` 常數,以及「靠幕索引猜哪幾支船參戰」的舊邏輯——現在哪幾支參戰完全由本欄資料決定。
...
`fx` 是宣告式特效事件陣列(取代以前寫死的 JS 閉包)。每個事件至少有 `at`(幕內秒)與 `type`,引擎在 `at` 秒呼叫對應特效。**只有以下五種 `type`**:
...
| `flood` | `to`(水面目標高度) | `dur`(上漲秒數,預設 6) | 水攻用:在 `at` 秒讓全圖水面從現高度漲到世界高度 `to`,歷時 `dur` 秒淹過低地。基準水面為 `0.05`;`to` 設多高取決於要淹的營/地形高度(結構地面約 0,營帳高度數個單位,`to:3~6` 通常足以沒過)。水位會跟著 scrub 倒退/前進正確還原。 |
...
1. 結構/契約驗:
   ...
   ```bash
   # 驗預設赤壁 package(packages/chibi/battlefield.json)
...
2. 人類播放時間軸(schema 過了不代表畫面對):在瀏覽器用 `play.html?pkg=<manifest>` 開該 package(預設赤壁可直接開 `play.html`,其他 package 要帶 `?pkg=` 指到它的 `battlefield.json`),逐幕走一遍,確認你改/加的幕在對的時間出現對的運鏡、事件卡、計策卡、特效(箭雨/火攻/震動/燒營),以及往回跳幕(scrub)時世界狀態正確。
