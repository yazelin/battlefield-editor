...
```js
const AUDIO = await (await fetch('packages/chibi/audio.json')).json();
```
...
> **素材路徑的 base(manifest 相對,P3 起已定案)**:`music.scenes`、`pathPattern`、`sfx.dir`、`narration.cues` 這些指向**素材**的路徑,引擎以 `assetUrl = PKG_BASE + 路徑` 解析,**相對 manifest 所在目錄**(與指向 `*.json` 的那層一致)。所以直接從 package 目錄寫起即可,例如 `"assets/narration/{voice}/scene{n}.mp3"`、`"cues": "assets/narration/cues.json"`;每個 package 自足擁有自己的 `assets/`。**別寫成 `packages/<名稱>/assets/...`** —— 會被 double-prefix 成 `packages/<名稱>/packages/<名稱>/assets/...`,執行期載不到、`audio-check` 也會以「路徑不存在」退件。(舊版本曾相對 document root,P3 抽 repo 後改為 manifest 相對,gaixia/guandu 即此寫法。)
