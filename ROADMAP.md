...
| P5.9(智慧換幕) | **自動播映依「內容做完」提早換幕,收掉尾巴空轉**:原本死等 `dur`(幾乎每幕空轉 5~11s)。改成偵測「旁白播完(`Narration.busy()`)+ 無單位在走(`U[*].path`)+ 無洪水在漲(`floodAnim`)+ 無待觸發事件(`pending`)」靜止滿 `STILL_HOLD`(1.8s)就切;環境效果(雨/火/水面)與相機運鏡不算動。`dur` 降為**上限**(絕不更晚)、每幕至少跑 `MIN_ACT`(2.5s)、可設 `hold:true` 不提早。六場 render 回歸綠;關旁白實測幕在內容+1.8s 切(非 dur)。SKILL/scene.md 同步「dur=上限、不必精算、hold 留停頓」。**SOP 反而變簡單**(不用再手算幕長) | ✅ ship |
...
### 暫停在這的待辦 + 未來方向(2026-06-19 記)

水攻線(全域 / 區域 / 水鋒)+ 可替換模型(結構/陸軍/水師 + 編輯器欄位)都已 ship。暫停時的未竟事項:

- [ ] **城池 glTF 模型搬入 —— 停在「找素材」**:管線已通,缺合用模型。候選 = Quaternius `Castle Fortress`
      ([poly.pizza/m/3r6gq3ow6O](https://poly.pizza/m/3r6gq3ow6O),**CC0**、歐式、適合先測管線)、
      Sketchfab `Low poly Chinese city`(中式但 **35 萬面 + CC-BY 要署名**,重)。**CC0 中式城樓稀少**。
      下一步 = 抓 CC0 的測管線(丟 `packages/<場>/assets/models/` + 設 `model`),或找/生/做中式城樓。
      `tools/make-test-glb.mjs` 可產測試素材。
- [ ] ~~**真・導向式洪水**~~(**多半 YAGNI,先不做**):現有**兩種**(① 全域整面漲 ② 區域+水鋒推進)已夠用,水鋒近似就有「潰堤流入」感。真流場(水有速度、繞地勢流而非橢圓擴張)邊際效益低、工程大;**除非哪天明確要「水繞著地形流動」的鏡頭才開**。
- [ ] **單位模型保留 glb 材質**:目前陸軍換模型只留陣營色 Lambert(丟原貼圖);要原材質需另接 per-instance 材質。
- [ ] **編輯器水位可播放**:`editWater` 是 sticky 靜態預覽,動完要重整 / 唯讀版才看得到洪水動畫;可加「播放本幕洪水」鈕。
- [ ] **地點標示牌(`marker`)不能在編輯器移動/編輯**:marker 是 `locLabel` 的 CSS2D 文字標籤、沒進 `STRUCT`,所以既不在「座標」選單也沒 mesh 可拖。修法 = 把 marker 納入 `epObj` 選單 + `fillXY`/`applyXY` 改 marker 的 CSS2D label 位置 + `structures.json` 的 x/z(或給 marker 一個隱形 hit mesh 才拖得到);順便可加改標籤文字。
- [ ] **事件卡手動擺位**:自動錯開已夠用;要精準放位再做拖曳 + 寫回 event x/z。
- [ ] **dogfood showcase**:**水淹下邳本身已是現成 showcase**(區域洪水 + 水鋒推進 + 完整 6 幕劇情)——不是缺口;只有想再做「另一場」代表作時才開。
- [ ] **新戰役**:關羽敗走麥城(**水淹七軍的直接後續** —— 同 219 關羽北伐:七軍=威震華夏巔峰、麥城=呂蒙白衣渡江斷後路→敗走被擒殞落;**與水淹下邳無關**),或最初列過的其他三國戰役。

### 適合的未來戰場候選(三國 / 戰術 / 計策 / 框得住;刻意不貪多)

- **夷陵(222)**:陸遜火燒連營七百里破劉備 —— **補齊「三大戰役」**(官渡 ✓ 赤壁 ✓ 夷陵);火攻有現成 `ignite`/`campFire`、計策=驕兵誘敵深入+以逸待勞、沿江連營好框。**最推**。
- **合肥・逍遙津(215)**:張遼率八百突襲、以寡擊眾威震逍遙津 —— 奇襲 + 守城,極戲劇、極戰術。
- **街亭(228)**:馬謖捨水上山、被張郃斷水源圍困而潰(諸葛亮一伐)—— **地形計(斷水圍山)是新機制變化**。
- (選配)**空城計(228)**:諸葛亮空城退司馬懿(演義)—— 純心理計、不流血的奇場,調性跟現有六場全不同。
