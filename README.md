...
- **宣告式特效/音效詞彙**:`volley`(箭雨)、`ignite`(燃燒)、`shake`(震動)、`campFire`(營火)、
  `flood`(水攻:水面動畫上升/退去,船隻隨水浮;可加 `region` 橢圓**只淹該區、其他水不漲**,省略=全域)… 由引擎的 dispatcher 執行;package 只宣告「何時、對誰」。
...
- **本幕洪水**:設每幕水**漲到多高 / 第幾秒起 / 漲幾秒 / 範圍**(`type:flood` + 可選 `region` 橢圓——只淹該區、其他水不漲;點圖設中心),拖「漲到」即時用水位預覽看 → 存回 scene。
