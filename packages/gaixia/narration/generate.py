#!/usr/bin/env python3
"""垓下旁白:edge-tts 雙聲 mp3 + 句級字幕 cue(SentenceBoundary 聚行)。
用法(需先 pip install edge-tts;此環境用 /tmp/ttsenv):
    /tmp/ttsenv/bin/python3 packages/gaixia/narration/generate.py        # 全部
    /tmp/ttsenv/bin/python3 packages/gaixia/narration/generate.py 7      # 只第 7 幕
輸出:packages/gaixia/assets/narration/<voice>/scene<N>.mp3 + cues.json
"""
...
# 垓下歌「騅不逝」的騅是罕用字(易吐空音),用同音錐替身;其餘罕用字盡量在 script 改白話避開。
SUBS = {
    "騅": "錐",   # zhuī 烏騅(項羽之馬);罕用字易吐空音,用同音錐替身
}
