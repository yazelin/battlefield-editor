...
- **scene**:多幕。每幕 `key/title/dur/env`(env ∈ day/cold/dusk/night/inferno/dawn)+ **非空 `shots`**(line/orbit/follow)+ `power`(key 為陣營)+ `set`/`march`/`fx`/`events`/`strat`/`combat`/`finale`。**`dur` 要 ≥ 該幕旁白音長**(否則旁白被切,audio-check 會擋);但 `dur` 只是**上限** —— 自動播映時引擎會在「旁白 + 動作 + 洪水 + 事件」都做完後自動提早換幕、收掉尾巴空轉,所以 **`dur` 設寬鬆即可、不必精算幕長**(要某幕刻意停久設 `hold:true`)。set/fx 引用的 unit/structure/camp 必須存在。
...
- **manifest `meta.intro`**(開場字卡,選填):`tag`/`bodyHtml`/`footHtml`。**`footHtml` 有通用預設(史/演義 tag 說明),不必寫**;要寫就**對齊該戰役朝代**——「演義」來源:三國→三國演義、楚漢→《史記》、東晉→《晉書》。**絕對別從赤壁複製** `classic.html`／`audition.html`／「無聲原版」／「選曲記錄」那兩個連結(那是 red-cliffs-3d 專屬頁,別包會死連結;residue-scan 會擋)。
...
- **「將」別緊接人名**:像「派降將朱序」會被斷成「派降 / 將朱序」、且「將」誤念 jiāng(江),糊成亂碼(「派翔醬」)。改用「**將領**」(中間「領」隔開人名:晉軍將領朱序)或重寫拿掉「將」(投降過來的朱序)。同理任何「X將+人名」(秦將梁成→秦軍將領梁成)都要避開。
...
- 配樂/音效:從音樂庫/音效庫指派(自動複製素材 + 寫 audio.json + 補 CREDITS):
  ```
  node tools/assign-music.mjs --list                          # 看音樂庫(74 首 CC0,依情緒分類 + fit)
  node tools/assign-music.mjs <slug> <一幕一首 id...>          # 依幕序指派配樂(--theme 另指定主題曲)
  node tools/assign-sfx.mjs   <slug> <id...>                   # 指派音效(audio.cues 的 name 要對上)
  ```
  目錄池(非 in-repo)的曲第一次用會從海選 stash 或 preview URL 取得(下載那次 Bash 要停 sandbox)。
  **庫不夠用?** append 一筆進 `tools/music-library.json` / `sfx-library.json`(id/mood/credit/preview),即刻全工具可用——這就是音樂/音效庫的擴充方式。
...
|---|---|
| schema 合法、跨檔引用完整、鏡頭參數齊全(line/orbit/follow)、audio cue 鍵與 payload 合法(validate-data) | 位置在不在合理的地方(空間) |
